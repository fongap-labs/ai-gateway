...
The heatmap data shape is intentionally mixed because it answers two different public questions:

```ts
Map<string, { total: number, requests: number }>
```

- `total` is physical upstream Token consumption reported by upstreams. Retry, protocol/model fallback, and hedge work remain part of Token consumption when the upstream reports usage.
- `requests` is **successfully delivered requests**. Internal retry/fallback/hedge dispatches do not increment the public `次请求` count by themselves.
- Model-usage breakdowns may separately expose physical upstream-call counts in tooltips. That physical-attempt dimension must not be substituted for the heatmap/public request count.
- Missing upstream Token usage is tracked separately and is never estimated merely to make the heatmap complete.

The current reader implements this split in `src/observability/token-usage-store/upstream-queries.ts`: Token fields come from `upstream_total_tokens`, while the activity-series request count comes from delivered `requests`.

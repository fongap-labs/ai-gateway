...
function renderModelRow(m: PublicModelStatusEntry, ttft: Map<string, TtftEntry> | null | undefined): string {
  ...
  const samplesText = t.noSamples ? '--' : String(t.samples);
  ...
  const sampleTitle = t.noSamples ? '暂无 TTFT 样本' : '近 24 小时成功请求的 TTFT samples';
  return `<div class="model-row">
    <div class="mr-name" title="${escapeHtml(stateTooltip)}">${escapeHtml(m.id)}</div>
    <div class="mr-status-cell" title="${escapeHtml(stateTooltip)}">
      <span class="mr-dot${style}"></span>
      <span class="mr-status">${state}</span>
    </div>
...
function renderModelBlock(title: string, models: PublicModelStatusEntry[], ttft: Map<string, TtftEntry> | null | undefined): string {
  if (!models.length) return '';
  const rows = models.map((m) => renderModelRow(m, ttft)).join('');
  return `<div class="status-panel">
    <div class="status-panel-head">
      <div class="status-panel-title">${escapeHtml(title)}</div>
      <div class="status-panel-note">服务状态 · TTFT (24h)</div>
    </div>
    <div class="model-table-head">
      <span>模型</span><span>状态</span><span>P50</span><span>P95</span><span>Samples</span>
    </div>
...
export function renderModels(status: { models?: PublicModelStatusEntry[] } | null | undefined, ttft: Map<string, TtftEntry> | null | undefined): { html: string } {
  ...
  const html = `<div class="status-grid">${renderModelBlock('通用模型', general, ttft)}${renderModelBlock('编程模型', code, ttft)}</div>`;
  return { html };
}

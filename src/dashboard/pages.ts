...
const FAVICON = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 48 48'><path d='M7 37L24 5L41 37Q33 40 24 31Q15 40 7 37Z' fill='none' stroke='%230f5d53' stroke-width='5' stroke-linecap='round' stroke-linejoin='round'/></svg>";
...
<meta name="description" content="AI Gateway — One endpoint. Built for upstream change.">
<title>${escapeHtml(title)}</title><link rel="icon" href="${FAVICON}"><style>${THEME_CSS}</style></head>
<body>
  ...
  <div class="wrap header-row">
    <a class="brand" href="#" aria-label="AI Gateway 首页">
      ...
      <span class="brand-copy">
        ...
        <span class="brand-slogan">One endpoint. Built for upstream change.</span>
      </span>
    </a>
    ${GH_ICON}
  </div>
...
export async function dashboardResponse(request: Request, env: Record<string, unknown>): Promise<Response> {
  try {
    ...
    return htmlResponse(shell({ title: 'AI Gateway', body }));
  } catch (e) {
    try { console.error('[dashboard] dashboardResponse error:', (e as Error)?.message || e, (e as Error)?.stack || ''); } catch {}
    const body = `
  <section class="fallback-section">
    ...
    return htmlResponse(shell({ title: 'AI Gateway', body }), { status: 500 });
  }
}

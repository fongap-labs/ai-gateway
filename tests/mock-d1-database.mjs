...
export function createMockD1({ failWrites = false, failReads = false } = {}) {
  ...
  const addUpstream = (cur, values) => {
    ...
    cur.upstreamEffectiveInput += effectiveInput || 0;
    cur.upstreamObservedRead += observedCacheRead || 0;
  ...
  const toSqlRow = (r) => ({
    ...
    upstream_effective_input_tokens: r.upstreamEffectiveInput,
    upstream_cache_observed_read_tokens: r.upstreamObservedRead,
    upstream_cache_observed_input_tokens: r.upstreamObservedInput,
    upstream_cache_read_reports: r.upstreamReadReports,
    ...
    const stmt = {
      ...
      async run() {
        ...
        if (/INSERT\s+INTO\s+token_usage_totals/i.test(sql)) {
          ...
          if (deliveredWrite) {
            ...
            if (this._params.length >= 21) addUpstream(totalsRow, this._params.slice(9, 21));
            else if (this._params.length >= 17) addUpstream(totalsRow, this._params.slice(9, 17));
          } else {
            if (this._params.length >= 13) {
              addUpstream(totalsRow, this._params.slice(0, 12));
              totalsRow.updated_at = this._params[12] || '';
            } else {
        ...
        if (/INSERT\s+INTO\s+token_usage_daily/i.test(sql)) {
          ...
          if (usesUpstream && this._params.length >= 21) {
            addDelivered(row, this._params.slice(1, 9));
            addUpstream(row, this._params.slice(9, 21));
          } else if (usesUpstream && this._params.length >= 17) {
            addDelivered(row, this._params.slice(1, 9));
        ...
        if (/token_usage_model_hourly/i.test(sql)) {
          ...
          if (/successful_ttft_count/i.test(sql)) {
            ...
            addUpstream(cur, this._params.length >= 14 ? this._params.slice(2, 14) : this._params.slice(2, 10));
          } else {
            addDelivered(cur, this._params.slice(2, 10));
          }
        ...
        if (usesUpstream && this._params.length >= 21) {
          ...
          addUpstream(cur, this._params.slice(9, 21));
        } else if (usesUpstream && this._params.length >= 17) {
          addDelivered(cur, this._params.slice(1, 9));
          addUpstream(cur, this._params.slice(9, 17));
        } else if (usesUpstream) {
          addUpstream(cur, this._params.length >= 13 ? this._params.slice(1, 13) : this._params.slice(1, 9));
        } else {
          addDelivered(cur, this._params.slice(1, 9));
        }
        rows.set(hour, cur);
      ...
      async first() {
        ...
        if (/FROM\s+token_usage_totals/i.test(sql)) {
          if (usesUpstream) {
            return {
              upstream_input_tokens: totalsRow.upstreamInput,
              upstream_output_tokens: totalsRow.upstreamOutput,
              upstream_cache_creation_input_tokens: totalsRow.upstreamCacheCreation,
              upstream_cache_read_input_tokens: totalsRow.upstreamCacheRead,
              upstream_effective_input_tokens: totalsRow.upstreamEffectiveInput,
              upstream_cache_observed_read_tokens: totalsRow.upstreamObservedRead,
              upstream_cache_observed_input_tokens: totalsRow.upstreamObservedInput,
              upstream_cache_read_reports: totalsRow.upstreamReadReports,
        ...
        if (/CASE\s+WHEN\s+hour\s*>=/i.test(sql)) {
          const todayStart = this._params[0];
          const h24Start = this._params[2];
          const isObservedShape = usesUpstream && /h24_cache_observed_input/i.test(sql);
          ...
          if (usesUpstream) {
            ...
            for (const [hour, r] of rows) {
              ...
              if (hour >= h24Start) {
                ...
                h24_input += r.upstreamEffectiveInput;
                h24_input_raw += r.upstreamInput;
                h24_cache_creation += r.upstreamCacheCreation;
                h24_cache_read += r.upstreamCacheRead;
                h24_cache_observed_read += r.upstreamObservedRead;
                h24_cache_observed_input += r.upstreamObservedInput;
        ...
        if (/SUM\((?:upstream_)?total_tokens\)|SUM\((?:upstream_)?attempts\)|SUM\(requests\)|SUM\((?:upstream_)?usage_reports\)|SUM\((?:upstream_)?usage_missing\)/i.test(sql)
          ...
          if (usesUpstream) {
            ...
            for (const r of rows.values()) {
              ...
              rp += r.upstreamReports;
              rm += r.upstreamMissing;
              input_raw += r.upstreamInput;
              cache_creation += r.upstreamCacheCreation;
              cache_read += r.upstreamCacheRead;
              output += r.upstreamOutput;
              effective_input += r.upstreamEffectiveInput;
              cache_observed_read += r.upstreamObservedRead;
              cache_observed_input += r.upstreamObservedInput;

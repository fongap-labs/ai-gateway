...
import {
  normalizeTokenUsage, normalizeUsageReport, recordTokenUsage, summarizeTokenStats,
...
const ENV = { GATEWAY_KEY_AIR: 'test-access-key', GATEWAY_MODELS_AIR: '*' };
...
await test('模型使用 shows official logical IDs, not lowercase statistics keys', async () => {
  ...
  env.TIER1_NODES_01 = JSON.stringify([{ id: 'node-a', provider: 'mock', base_url: 'https://a.example.com/v1', models: { 'Code-Max': 'up-max', 'Code-Ultra': 'up-ultra' } }]);
  env.TIER1_CREDENTIALS_01 = JSON.stringify({ 'node-a': 'test-key' });
...
await test('模型状态 section has model rows with status, P50, P95, sample count', async () => {
  __resetDashboardCacheForTests(); const d1 = createMockD1(); const env = deepClone(ENV); env.TOKEN_STATS_DB = d1; env.TIER1_NODES_01 = JSON.stringify([{ id: 'node-a', provider: 'mock', base_url: 'https://a.example.com/v1', models: { 'max': 'up-max' } }]); env.TIER1_CREDENTIALS_01 = JSON.stringify({ 'node-a': 'test-key' }); const html = await pageText(anonRequest(), env); assert.ok(html.includes('P50')); assert.ok(html.includes('P95')); assert.ok(html.includes('samples')); assert.ok(html.includes('mr-status')); assert.ok(html.includes('status-grid'));
});
